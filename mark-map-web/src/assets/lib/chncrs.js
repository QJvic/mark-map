...
/* https://github.com/maptalks/maptalks.js/issues/949 */
...
var ProjectionTransform = {
  ...
  _transformGeoJSON: function (geoJSON, fromCRS, toCRS) {
    ...
    var result = this._extend({}, geoJSON);
    var coordinates = this._transformCoordinate(
      geoJSON['coordinates'],
      fromCRS,
      toCRS
    );
  ...
  _isCoord: function (coordinate) {
    if (
      this._isArray(coordinate) &&
      this._isNumber(coordinate[0]) &&
      this._isNumber(coordinate[1])
    ) {
      return true;
    }
    return false;
  ...
  _isArray: function (obj) {
    ...
    return (
      typeof obj == 'array' ||
      (obj.constructor !== null && obj.constructor == Array)
    );
  },
  ...
  _isString: function (_str) {
    ...
    return (
      typeof _str == 'string' ||
      (_str.constructor !== null && _str.constructor == String)
    );
  }
};
