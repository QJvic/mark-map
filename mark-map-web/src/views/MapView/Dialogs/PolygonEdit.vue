          ...
          <template #reference>
            <div>
              <svg
                ...
                height="100"
              >
                <polygon points="10,10,10,90,90,90,90,10" :style="svgStyle" />
              </svg>
            ...
            <div>
              ...
              <div class="my-[15px]">
                ...
                <el-color-picker
                  size="small"
                  show-alpha
                  v-model="editingStore.symbol.polygonFill"
                  :predefine="fillPredefineColors"
                  @change="
                    val =>
                      val ||
                      (editingStore.symbol.polygonFill = fillPredefineColors[1])
                  "
                />
              </div>
...
const editingStore = useEditingPolygonStore();
...
const svgStyle = computed(() => {
  let str = '';
  str += `fill: ${editingStore.symbol.polygonFill};`;
...
async function onOpen() {
  editingStore.symbol = {
    ...
    polygonFill: fillPredefineColors.value[0],
    lineWidth: 3,
    lineDasharray: null,
    ...editingStore.symbol
  };
...
async function onConfirmClick() {
  ...
  const map = getMap();
  if (editingStore.id) {
    await map.mm.editPolygon(editingStore);
  } else {
    await map.mm.addPolygon(editingStore);
  }
  onCancelClick();
}
